...
export default function ClientDashboard() {
  ...
  useEffect(() => {
    (async () => { await loadData(); setInitialLoading(false); })();
  }, []);
  ...
  const loadData = async () => {
    try {
      ...
      const todays = comps.filter(c => {
        const d = typeof c['Completion Date'] === 'string'
          ? c['Completion Date'].split('T')[0]
          : format(new Date(c['Completion Date']), 'yyyy-MM-dd');
        return d === todayStr;
      });
  ...
  const markAllComplete = () => {
    ...
    const next = {}; assignList.forEach(a => (next[a.id] = true));
    setSelections(next); vibrate(18);
  };
  const clearAll = () => {
    const next = {}; assignList.forEach(a => (next[a.id] = false));
    setSelections(next);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      for (const aid of Object.keys(selections)) {
        const selected = selections[aid];
        const has = aid in todayMap;
  ...
  return (
    <div>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ width: '100%', height: 30, background: '#e9ecef', borderRadius: 15, overflow: 'hidden' }}>
              <div style={{
                ...
                height: '100%',
                background: completionPercentage === 100 ? '#28a745' : '#007bff',
                transition: 'width .3s'
              }} />
            </div>
      ...
      {/* No extra text link; bottom tabs are enough for navigation */}
      ...
      {showCelebration && (
        <div style={{
          ...
          zIndex: 9998, background: 'var(--success)', color: '#fff', padding: 'var(--sp-3) var(--sp-4)',
          borderRadius: 'var(--radius)', boxShadow: 'var(--shadow)', fontSize: 16, fontWeight: 600, textAlign: 'center', maxWidth: '90vw'
        }} aria-live="polite" role="status">
